...
import { purgeInboxEventUseCase } from "./purge-inbox-event.use-case.js";
...
describe("purgeInboxEventUseCase", () => {
  ...
  it("answers with nothing once the row is purged", async () => {
    ...
    expect(await purgeInboxEventUseCase(aCommand())).toBeUndefined();
  });

  it("does not read the status again on the happy path", async () => {
    purgeById.mockResolvedValue(true);

    await purgeInboxEventUseCase(aCommand());
  ...
  it("stores the operator verbatim", async () => {
    ...
    await purgeInboxEventUseCase(aCommand({ by: "donatas" }));
  ...
  it("404s when the conditional update matched nothing and the row is gone", async () => {
    ...
    await expect(purgeInboxEventUseCase(aCommand())).rejects.toMatchObject({
...
describe("purgeInboxEventUseCase transaction", () => {
  ...
  it("records the purge against the event, naming actor, caller and reason", async () => {
    ...
    await purgeInboxEventUseCase(aCommand({ by: "donatas", caller: "gas" }));
  ...
  it("records the operator as the audit actor", async () => {
    ...
    await purgeInboxEventUseCase(aCommand({ by: "ada" }));
