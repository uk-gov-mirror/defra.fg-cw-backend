...
import { purgeOutboxEventRoute } from "./purge-outbox-event.route.js";

vi.mock("../use-cases/purge-outbox-event.use-case.js");

const ID = "665f1c2e9a1b2c3d4e5f6a7b";

const validateParams = (params) =>
  purgeOutboxEventRoute.options.validate.params.validate(params);

const validatePayload = (payload) =>
  purgeOutboxEventRoute.options.validate.payload.validate(payload);

const validateQuery = (query) =>
  purgeOutboxEventRoute.options.validate.query.validate(query);

const handle = async (request) => {
  purgeOutboxEventUseCase.mockResolvedValue(undefined);
  const code = vi.fn().mockReturnValue("no-content");
  const h = { response: vi.fn().mockReturnValue({ code }) };

  const result = await purgeOutboxEventRoute.handler(
    {
      params: { id: ID },
      query: { by: "donatas" },
...
describe("purgeOutboxEventRoute", () => {
  it("is a POST on /actuators/events/outbox/{id}/purge", () => {
    expect(purgeOutboxEventRoute.method).toBe("POST");
    expect(purgeOutboxEventRoute.path).toBe(
      "/actuators/events/outbox/{id}/purge",
    );
  });

  it("is on the public-api strategy", () => {
    expect(purgeOutboxEventRoute.options.auth).toBe("public-api");
  });

  it("is tagged for the public API surface", () => {
    expect(purgeOutboxEventRoute.options.tags).toEqual(["api", "public-api"]);
  });
  ...
  it("passes the id, the reason and the note to the use case", async () => {
    ...
    expect(purgeOutboxEventUseCase).toHaveBeenCalledWith({
      id: ID,
      by: "donatas",
...
describe("purgeOutboxEventRoute actor", () => {
