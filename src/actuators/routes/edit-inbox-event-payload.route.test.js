...
import { editInboxEventPayloadRoute } from "./edit-inbox-event-payload.route.js";

vi.mock("../use-cases/edit-inbox-event-payload.use-case.js");

const ID = "665f1c2e9a1b2c3d4e5f6a7b";

const { options } = editInboxEventPayloadRoute;
...
const handle = async (request = {}) => {
  editInboxEventPayloadUseCase.mockResolvedValue({
    payloadRevision: 1,
  ...
  return editInboxEventPayloadRoute.handler({
    params: { id: ID },
    query: { by: "donatas" },
    payload: aBody(),
...
describe("editInboxEventPayloadRoute", () => {
  it("is a POST on /actuators/events/inbox/{id}/payload", () => {
    expect(editInboxEventPayloadRoute.method).toBe("POST");
    expect(editInboxEventPayloadRoute.path).toBe(
  ...
  it("passes the id, the operator, the caller and the body to the use case", async () => {
    ...
    expect(editInboxEventPayloadUseCase).toHaveBeenCalledWith({
      id: ID,
      by: "donatas",
